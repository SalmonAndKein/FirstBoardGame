...
bool ChessBoard::AddTileTo(BoardTile* _tile,int _x, int _y)
{
    if(tiles[_y][_x] == NULL)
    {
...
}void ChessBoard::ResetBoard()
{
    ...
    for(int y=0; y<8; y++)
    {
        for(int x=0; x<8; x++)
        {
            if(str[num]>='A' && str[num]<='Z')
            {
                switch (str[num])
                {
                    case 'P':
                    {
                        auto _tile = BoardTile::createBoardTile(*"W1.png", 'P', 2);
                        AddTileTo(_tile, x, y);
                        break;
                    }
                    case 'R':
                    {
                        auto _tile = BoardTile::createBoardTile(*"W2.png", 'R', 2);
                        AddTileTo(_tile, x, y);
                        break;
                    }
                    case 'N':
                    {
                        auto _tile = BoardTile::createBoardTile(*"W3.png", 'N', 2);
                        AddTileTo(_tile, x, y);
                        break;
                    }
                    case 'B':
                    {
                        auto _tile = BoardTile::createBoardTile(*"W4.png", 'B', 2);
                        AddTileTo(_tile, x, y);
                        break;
                    }
                    case 'Q':
                    {
                        auto _tile = BoardTile::createBoardTile(*"W5.png", 'Q', 2);
                        AddTileTo(_tile, x, y);
                        break;
                    }
                    case 'K':
                    {
                        auto _tile = BoardTile::createBoardTile(*"W6.png", 'K', 2);
                        AddTileTo(_tile, x, y);
                        break;
                    }
                    default:
            ...
            if(str[num]>='a' && str[num]<='z')
            {
                switch (str[num])
                {
                    case 'p':
                    {
                        auto _tile = BoardTile::createBoardTile(*"B1.png", 'p', 1);
                        AddTileTo(_tile, x, y);
                        break;
                    }
                    case 'r':
                    {
                        auto _tile = BoardTile::createBoardTile(*"B2.png", 'r', 1);
                        AddTileTo(_tile, x, y);
                        break;
                    }
                    case 'n':
                    {
                        auto _tile = BoardTile::createBoardTile(*"B3.png", 'n', 1);
                        AddTileTo(_tile, x, y);
                        break;
                    }
                    case 'b':
                    {
                        auto _tile = BoardTile::createBoardTile(*"B4.png", 'b', 1);
                        AddTileTo(_tile, x, y);
                        break;
                    }
                    case 'q':
                    {
                        auto _tile = BoardTile::createBoardTile(*"B5.png", 'q', 1);
                        AddTileTo(_tile, x, y);
                        break;
                    }
                    case 'k':
                    {
                        auto _tile = BoardTile::createBoardTile(*"B6.png", 'k', 1);
                        AddTileTo(_tile, x, y);
                        break;
                    }
                    default:
        ...
        for (int y=2; y<6; y++)
        {
            for(int x=0; x<8; x++)
            {
                auto _tile = BoardTile::createEmptyTile();
                AddTileTo(_tile, x, y);
            }
        }
    }
...
BoardTile* ChessBoard::GetTile(int x, int y)
{
    if( CheckIndex(x, y) )
    {
        return tiles[y][x];
...
BoardTile* ChessBoard::GetTile(cocos2d::Vec2 &pos)
{
